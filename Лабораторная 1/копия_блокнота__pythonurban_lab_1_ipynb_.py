...
Original file is located at
    https://colab.research.google.com/drive/1_NYl_Qh5CZkYRHDJLDQZ7JcJUA6YqH3S
...
def task_1(list_cities):  # TODO написать функцию task_1
    return [city.lower().title() for city in list_cities]
...
def task_2(list_dict_cities, filter_population): # TODO написать функцию task_2
    spisok = [city for city in list_dict_cities if city['population'] >= filter_population]
    return spisok
...
def task_3(fisrt_group, second_group):  # TODO Написать функцию task_3
    return [city for city in fisrt_group if city in second_group]
...
def task_4(list_cities, find_city):  # написать функцию task_4
    return [city.lower() for city in list_cities].count(find_city.lower())
...
def task_index_green(list_territories): # TODO написать функцию, которая будет высчитывать индексы озеленения для каждой территории
    new_list_territories = [{"territory_name": city["territory_name"], 
                             "index_green": (sum(city["green_zones"])/city["territory_area"])} 
                            for city in list_territories]
    return new_list_territories

def task_average(dic_indexs): # TODO написать функцию, которая будет высчитывать средний индекс озеленения по всем территориям
    indexs = [city["index_green"] for city in dic_indexs]
    return sum(indexs)/len(indexs)

print(round(task_average(task_index_green(list_territories)), 4)) # TODO распечатать результат округленный до 4 знаков после запятой